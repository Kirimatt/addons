...
func ScheduleDownloadingAddons(placeholderUrl string) error {
	go func() {
		for {
			fmt.Println("Started scheduled task")
			err := scheduledDownloadingAddons(placeholderUrl)
			if err != nil {
				return
			}
...
func scheduledDownloadingAddons(placeholderUrl string) error {
	orderToUrlDescription, err := http.GetUrlsFromSearch(os.Getenv("SEARCH_URL"), placeholderUrl)
	if err != nil {
