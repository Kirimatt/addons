...
func scheduledDownloadingAddons(searchUrl string, databaseUrl string, placeholderUrl string) error {
	...
	for orderId, urlDescription := range *orderToUrlDescription {
		processUrl(*postgres, orderId, urlDescription)
		if err != nil {
...
func processUrl(postgres pg.Postgres, orderId int, urlDescription http.UrlDescription) error {
	...
	govermentAddon := pg.GovermentAddon{Url: *finalUrl, Status: urlDescription.Status, Text: *res, OrderId: orderId}
