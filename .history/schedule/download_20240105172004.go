...
func ScheduleDownloadingAddons(searchUrl string, placeholderUrl string) error {
	go func() {
		for {
			fmt.Println("Started scheduled task")
			err := scheduledDownloadingAddons(searchUrl, placeholderUrl)
			if err != nil {
				return
			}
...
func scheduledDownloadingAddons(searchUrl string, placeholderUrl string) error {
