...
func scheduledTask(searchUrl string, databaseUrl string, placeholderUrl string) error {
	...
	for order, urlDescription := range *orderToUrlDescription {
		processUrl(*postgres, order, urlDescription)
	}

	return nil
}

func processUrl(postgres pg.Postgres, order int, urlDescription http.UrlDescription) error {
	...
	govermentAddon := pg.GovermentAddon{Url: *finalUrl, Status: urlDescription.Status, Text: *res, Order: order}
