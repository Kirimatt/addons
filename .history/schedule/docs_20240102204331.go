...
func ScheduleDownloadingDocs(searchUrl string, databaseUrl string, placeholderUrl string) error {
	...
	id, err := scheduler.Add(&tasks.Task{
		...
		TaskFunc: func() error {
			return ScheduledTask(searchUrl, databaseUrl, placeholderUrl)
		},
	})
	fmt.Printf("Started task with id: %s", id)
	if err != nil {
		return fmt.Errorf("Failed task with error: %w and id: %s", err, id)
	}
